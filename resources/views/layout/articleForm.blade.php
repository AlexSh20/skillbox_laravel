...
<div class="form-group">
    ...
    <input type="text" class="form-control" id="inputName" name="name"
           placeholder="Добавьте название статьи" value="{{ old('name', $slug->name ?? '') }}">
...
<div class="form-group">
    ...
    <textarea class="form-control" id="inputDescription" name="description" rows="3"
              placeholder="Краткое описание">{{ old('description', $slug->description ?? '') }}</textarea>
</div>

<div class="form-group">
    <label for="inputBody">Текст</label>
    <textarea class="form-control" id="inputBody" name="text"
              rows="3">{{ old('text', $slug->text ?? '') }}</textarea>
</div>
